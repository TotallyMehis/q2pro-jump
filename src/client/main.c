...
static void CL_UpdateGunSetting(void) {
  ...
  if (cl_gun->integer == -1) {
    nogun = 2;
  } else if (cl_gun->integer == 0 ||
             (info_hand->integer == 2 && cl_gun->integer == 1)) {
    nogun = 1;
  } else {
    nogun = 0;
  }
...
void CL_CheckForResend(void) {
  ...
  case PROTOCOL_VERSION_R1Q2:
    Q_snprintf(tail, sizeof(tail), " %d %d", maxmsglen,
               PROTOCOL_VERSION_R1Q2_CURRENT);
    cls.quakePort = net_qport->integer & 0xff;
    break;
  case PROTOCOL_VERSION_Q2PRO:
    Q_snprintf(tail, sizeof(tail), " %d %d %d %d", maxmsglen,
               net_chantype->integer, USE_ZLIB, PROTOCOL_VERSION_Q2PRO_CURRENT);
    cls.quakePort = net_qport->integer & 0xff;
    break;
  default:
    tail[0] = 0;
  ...
  Netchan_OutOfBand(NS_CLIENT, &cls.serverAddress,
                    "connect %i %i %i \"%s\"%s\n", cls.serverProtocol,
                    cls.quakePort, cls.challenge, userinfo, tail);
}
...
static void CL_ParseStatusResponse(serverStatus_t *status, const char *string) {
  ...
  qsort(status->players, status->numPlayers, sizeof(status->players[0]),
        SortPlayers);
}
...
static void CL_DumpStatusResponse(const serverStatus_t *status) {
  ...
  for (i = 0; i < status->numPlayers; i++) {
    Com_Printf("%3i %5i %4i %s\n", i + 1, status->players[i].score,
               status->players[i].ping, status->players[i].name);
  }
}
      ...
      if (!strncmp(s, "ac=", 3)) {
        ...
        if (*s) {
          ...
          if (type != NETCHAN_OLD && type != NETCHAN_NEW) {
            Com_Error(ERR_DISCONNECT, "Server returned invalid netchan type");
          }
        }
...
void CL_LoadFilterList(string_entry_t **list, const char *name,
                       const char *comments, size_t maxlen) {
...
static void CL_LoadStuffTextWhiteList(void) {
  CL_LoadFilterList(&cls.stufftextwhitelist, "stufftext-whitelist.txt", NULL,
                    MAX_STRING_CHARS);
}
...
static void CL_DumpLayout_f(void) { dump_program(cl.layout, "layout"); }
...
static size_t CL_Texture_m(char *buffer, size_t size) {
  ...
  int mask = CONTENTS_SOLID | CONTENTS_WINDOW | CONTENTS_WATER |
...
====================
*/
static void CL_ReloadRefresh_f(void) { CL_RestartRefresh(false); }
...
static void CL_RestartRefresh_f(void) {
  ...
  if (!cl_allow_vid_restart->integer && strcmp(Cmd_Argv(1), "force")) {
    ...
    Com_Printf(
        "Video settings are automatically applied by " PRODUCT ", thus manual "
...
static void cl_gun_changed(cvar_t *self) { CL_UpdateGunSetting(); }

static void info_hand_changed(cvar_t *self) { CL_UpdateGunSetting(); }

static void cl_gibs_changed(cvar_t *self) { CL_UpdateGibSetting(); }

static void cl_footsteps_changed(cvar_t *self) { CL_UpdateFootstepsSetting(); }

static void cl_predict_changed(cvar_t *self) { CL_UpdatePredictSetting(); }

#if USE_FPS
static void cl_updaterate_changed(cvar_t *self) { CL_UpdateRateSetting(); }
#endif

static void cl_flares_changed(cvar_t *self) { CL_UpdateFlaresSetting(); }
...
static void cl_sync_changed(cvar_t *self) { CL_UpdateFrameTimes(); }
...
static const cmdreg_t c_client[] = {
    ...
    //
    // jump mod specific server commands
    //
    {"team"},
...
static void CL_InitLocal(void) {
  ...
  Cmd_AddMacro("cl_fps", CL_Mps_m, NULL);
  ...
  Cmd_AddMacro("cl_numentities", CL_NumEntities_m, NULL);
...
static void CL_CheckTimeout(void) {
  ...
  if (cl_timeout->integer &&
      com_localTime - cls.netchan.last_received > cl_timeout->integer) {
...
#if USE_DEBUG
static const char *const sync_names[] = {"SYNC_TIMEDEMO", "SYNC_MAXFPS",
                                         "SYNC_SLEEP_10", "SYNC_SLEEP_60",
                                         "ASYNC_FULL"};
#endif
...
void CL_UpdateFrameTimes(void) {
  ...
  Com_DDPrintf("%s: mode=%s main_msec=%d ref_msec=%d, phys_msec=%d\n", __func__,
               sync_names[sync_mode], main_msec, ref_msec, phys_msec);
}
...
unsigned CL_Frame(unsigned msec) {
  ...
  Com_DDDDPrintf("main_extra=%d ref_frame=%d ref_extra=%d "
                 ...
                 main_extra, ref_frame, ref_extra, phys_frame, phys_extra);
